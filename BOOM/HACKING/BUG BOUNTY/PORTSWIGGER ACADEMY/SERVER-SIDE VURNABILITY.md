...
It is used as a technique that can read files on the server on which the website is running(reaching to us )
We can read the data, modify and get the sensitive files or code

Suppose we are visiting a site that loads up a image <span style="color:rgb(0, 176, 240)">img src="/loadImage?filename=218.png</span> but on backend it asks the file from /var/www/images/218.png on the linux server.

 <span style="color:rgb(0, 176, 240)">https://insecure-website.com/loadImage?filename=../../../etc/passwd</span> will load the data without any blocking
...
*Lab Solution*:
payload =<span style="color:rgb(0, 176, 240)"> ../../../etc/passwd</span>

## Access Control

It gives certain permissions to certain users like to authenticate them, allow verified user to make changes and session management.
![[Pasted image 20240405104208.png]]
## Vertical Privilege Escalation

It is the condition when user are given some permission which they are not supposed to be given.
...
The are sometimes when the website do not block the files which can be found by fuzzers like ffuf.
Example : <span style="color:rgb(0, 176, 240)">https://target.com/robots.txt</span>
        <span style="color:rgb(0, 176, 240)">https://target.com/admin</span> -- > gives path to admin panel

*<span style="color:rgb(192, 0, 0)">Lab Solution</span>*:
visit /robots.txt
then administrator-panel and do the delete part manually

## Unprotected functionality - Continued

Sometimes the admin panel is not easily to be found like visiting<span style="color:rgb(0, 176, 240)"> https://target.com/robots.txt</span> .
A another approach is to find its location in the source code.

Example:
  <span style="color:rgb(0, 176, 240)">https://insecure-website.com/administrator-panel-yb556</span>

![[Pasted image 20240405111101.png]]
 You can search the keyword admin or something to find the path.

*<span style="color:rgb(192, 0, 0)">Lab Solution</span>*:
click on every link or path 
then look for admin keyword in response(pretty) 
then you would found  something like this adminPanelTag.setAttribute('href', '<span style="color:rgb(0, 176, 240)">/admin-v7bzn6</span>');
then visit <span style="color:rgb(0, 176, 240)">/admin-v7bzn6</span>
then delete the carlos user manually 
...
![[Pasted image 20240405201138.png]]

The user-controllable location can be a cookie, hidden field or some query.



*<span style="color:rgb(192, 0, 0)">Lab Solution</span>:* 
intercept the proxy by visiting <span style="color:rgb(0, 176, 240)">/admin/delete?username=carlos</span>
and change admin to true or false 

## Horizontal privilege escalation

Horizontal privilege escalation occurs if a user is able to gain access to resources belonging to another user, instead of their own resources of that type. For example, if an employee can access the records of other employees as well as their own, then this is horizontal privilege escalation.
example -<span style="color:rgb(0, 176, 240)"> https://insecure-website.com/myaccount?id=123</span>
This is an example of an insecure direct object reference (IDOR) vulnerability. This type of vulnerability arises where user-controller parameter values are used to access resources or functions directly.

<span style="color:rgb(192, 0, 0)">Lab Solution</span>:
open the blog post made by the user wiener and intercept the requests. You will find /web-security-academy.net/blogs?userId=<span style="color:rgb(0, 176, 80)">4d154d65-eb1d-484b-9824-9321d4f3e224 </span>here the green part is the GUID(Globally Unique IDentifier) .
Login into the wiener account and intercept the request and you will get  /my-account?id=<span style="color:rgb(0, 176, 80)">4d154d65-eb1d-484b-9824-9321d4f3e224</span> 
So we get to know we just need to find the GUID to get into Carlos /my-account so we find the post made by the user Carlos /web-security-academy.net/blogs?userId=<span style="color:rgb(0, 176, 80)">d875acc0-54e4-49b0-b3e1-d4e1e7471455
</span> and then make a request /my-account?id=<span style="color:rgb(0, 176, 80)">d875acc0-54e4-49b0-b3e1-d4e1e7471455</span> .
And then submit the API --  g81kWIyDZbHpdsY91xrnbqeiYUZ7PbWA
...
<span style="color:rgb(255, 0, 0)">Solution :</span>

Login with the weiner id and password 
check the url it will have something like this /my-account?id=weiner
change the weiner to carlos 
 /my-account?id=carlos
check the hidden value of password in the  hidden field which can be seen by using the inspect in right click of mouse you will find "t4387ng1i78ar0ud1cad" for carlos
now then
check for `administrator`  /my-account?id=administrator
you will find the value `f1cm1zg0v00o8bhb6qzu`
Now simply past the id and password of the administrator and login to it 
Then visit the admin panel and delete the user carlos from it.
...
![[Pasted image 20260330220357.png]]
...
Just login with wiener id and password 
and check the exploit server you will find out the mail just fill the code to get into account and then 
just login into carlos then ek baar back karde hojaga lab complete.
...
![[Pasted image 20260331002352.png]]
...
![[Pasted image 20260331002531.png]]
...
Visit the product section and check for number of stocks available for it
Intercept the request and check the parameter for StockApi to ``http://localhost/admin``
Then you will get the delete users on web ui and the inspect the element of delete carlos
then ( you will find it  by using inspect )
change the StockApi to `http://localhost/admin/`delete?username=carlos``
...
![[Pasted image 20260331004054.png]]
...
Change the StockApi to `http://192.168.0.fuzz:8080`
fuzz the number you will get 200 OK at `147`
then simply see the site and check the url of delete user by inspecting it and then paste it to the url
...
![[Pasted image 20260331005743.png]]

## Lab: Remote code execution via web shell upload

<span style="color:rgb(255, 0, 0)">Lab Solution :</span>

Login with the wiener profile 
You will find the upload profile button there use it to upload a shellcode written in php and upload it  and remember the name of the file 
`<?php echo file_get_contents('/home/carlos/secret'); ?>' paste it in the file 
and upload 
...
Then submit the secret code and solve the lab
...
![[Pasted image 20260331142931.png]]
...
Login with the weiner account 
and then upload the php file with the shell code 
`<?php echo file_get_contents('/home/carlos/secret'); ?>'
It will fail so change the content-type 
from Content-Type: application/octet-stream to Content-Type: image/jpeg
Then it will be uploaded after that visit 
/files/avatars/exploit.php and get the secret code `oxFa7tl34FON3kIJJR1XBM5xGHdBn280`
...
Check stock of any product and inspect the request 
visit POST /product/stock
productId=1|whoami&storeId=1 
you will solve the lab
...
Check the stock of any section and 
the program like 
gifts'+OR+1=1--
...
Visit my account page and
then login with the id and password
id = administrator'-- & password can by anything
`'--`  makes the additional checking like password unnecessary .
